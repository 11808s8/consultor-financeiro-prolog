...
opc(X):-
    X = 6,
    write("Nome do cliente (Exemplo: homer_simpson): "),
    nl,
    read(C),
    write("Saldo do cliente:"),
    nl,
    ...
    nl,
    read(PRAZO_RETORNO),
    write("Aplicações (Exemplo: [tesouro_selic,cdb,lci]):"),
    nl,
    read(APLICACOES),
    write("Quantia em cada aplicacao (Na mesma ordem que as aplicações. Exemplo: [30,1000,20]):"),
    nl,
    read(QUANTIA),
    write("Perfil de investidor (Perfis disponiveis: nenhum; agressivo; conservador; moderado):"),
    nl,
    read(PERFIL),
    write("Despesas mensais:"),
    nl,
